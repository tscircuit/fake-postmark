...
const sendEmailWithTemplateRequestBodySchema = emailBaseSchema.and(
  z.union([
    z.object({ TemplateId: z.number().int(), TemplateAlias: z.string().optional() }), // TemplateId is present
    z.object({ TemplateId: z.number().int().optional(), TemplateAlias: z.string() })  // TemplateAlias is present
  ])
);
...
export default withRouteSpec({
  methods: ["POST"],
  jsonRequestBody: sendEmailWithTemplateRequestBodySchema,
  ...
  const dbInput: InputForTemplatedEmail = {
    ...
    ...(emailDataFromRequest.TemplateId !== undefined && { TemplateId: emailDataFromRequest.TemplateId }),
    ...(emailDataFromRequest.TemplateAlias !== undefined && { TemplateAlias: emailDataFromRequest.TemplateAlias }),
  };
